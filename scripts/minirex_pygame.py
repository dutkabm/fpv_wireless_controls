...
def get_channels_from_joystick(joystick, axis_channel_map, button_channel_map, hat_channel_map):
    ...
    # The game loop calls pygame.event.get(), which pumps; the serial writer thread skips pump.
...
def main():
    ...
    if os.path.exists(config_file):
        ...
        if 'General' in config:
            ...
            _ji = str(general.get('joystick_index', fallback='')).strip().lower()
            if _ji == 'auto':
                pygame.joystick.init()
                joystick_index = 0 if pygame.joystick.get_count() > 0 else None
            else:
    ...
    prev_serial = selected_serial_port
    selected_serial_port = autodetect_serial_port(selected_baud_rate, selected_serial_port)
    if selected_serial_port is None:
        selected_serial_port = default_serial_port
    elif (
        not is_autoselect_serial_port(prev_serial)
        and str(prev_serial).strip() != str(selected_serial_port).strip()
    ):
        print(f"Configured serial port {prev_serial!r} missing; using {selected_serial_port!r}.")
